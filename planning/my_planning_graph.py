...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        cur_s_nodes = self.s_levels[level]
        for a in actions:
            # construct the action node from given action
            a_node = PgNode_a(a)
            prev_nodes = a_node.prenodes
            if prev_nodes.issubset(cur_s_nodes):
                for s_node in cur_s_nodes:
                    s_node.children.add(a_node)
                    a_node.parents.add(s_node)
                a_nodes_list.append(a_node)
    ...
    def add_literal_level(self, level):
        ...
        # get list of S nodes from the effect nodes of previous level A
        prev_a_nodes = self.a_levels[level - 1]
        for prev_a_node in prev_a_nodes:
            s_nodes = prev_a_node.effnodes
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pre_cond_a1 in node_a1.parents:
            for pre_cond_a2 in node_a2.parents:
                if pre_cond_a1.is_mutex(pre_cond_a2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        for pre_cond_s1 in node_s1.parents:
            for pre_cond_s2 in node_s2.parents:
                if not pre_cond_s1.is_mutex(pre_cond_s2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for sub_goal in goals:
            levels = range(len(self.s_levels))
            for level in levels:
                s_nodes = self.s_levels[level]
                for s in s_nodes:
                    if sub_goal == s.symbol and not found:
                        level_sum += level + 1
                        found = True
                        break
        return level_sum
